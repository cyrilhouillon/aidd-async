...
A config object carrying the two trigger tags, the Anthropic auth mode and its secret, and the plugin marketplaces with the framework access.
...
4. **Emit.** Hand the config to `03-generate`; persist nothing yet.

## Test

- The config carries both tags, an auth mode that is `api_key` or `oauth_token` with its secret name, and the two marketplaces with the framework access resolved from the detection probe.
